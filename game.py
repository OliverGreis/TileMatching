...
class Game():
    ...
    def detect_globe(self, auto = False):
            for x in range(1, len(self.grid)-1):
                for y in range(0, len(self.grid)):
                    if self.grid[x][y] == 7:
                        print(x)
                        if self.grid[x][y] == random(1,5):
                            self.grid[x][y] = 0
        ...
        for x in range(1, len(self.grid)-1):
            for y in range(0, len(self.grid)):
                ...
                if self.grid[x][y] == self.grid[x-1][y] and self.grid[x][y] == self.grid[x+1][y]:
                    if self.grid[x][y] == self.grid[x-1][y] and self.grid[x][y] == self.grid[x+1][y] and self.grid[x-1][y] and self.grid[x-2][y] or self.grid[x+2][y] and self.bombeone == False :
                        self.grid[randint(0,9)][randint(0,9)] = 6
                        self.bombeone = True
                    if self.grid[x][y] == self.grid[x-1][y] and self.grid[x][y] == self.grid[x+1][y] and self.grid[x-1][y] and self.grid[x+2][y] and self.grid[x-2][y]:
                        self.grid[randint(0,9)][randint(0,9)] = 7
                    c = self.grid[x][y]
                    self.grid[x-1][y] = 0
                    self.grid[x][y] = 0
                    self.grid[x+1][y] = 0
